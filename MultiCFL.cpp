...
int MultiCFL::NumNodes() const
{
	return interfereGraph.Count_of_Vertices();
}

vector<unordered_set<int>> MultiCFL::GetCurrentColoring() const
{
	return currentColoring;
}

vector<vector<double>> MultiCFL::GetCurrentProbability() const
{
	return currentProbability;
}

MultiCFL::MultiCFL(int _numFragment, int _numColors, Graph _interfereGraph):numFragment(_numFragment),interfereGraph(_interfereGraph),numColors(_numColors),num_total_conflicts(0)
{
...
bool MultiCFL::RunAlgorithm(int Max_Cycle, double beta, int window)
{
...
bool MultiCFL::IsDecreasing(vector<int> series,int window)
{
...
void MultiCFL::PickupRandomColoring()
{
	//randomly pickup numFragment choices from all fractional colors
	...
	for(int i=0;i<num_nodes;i++){
		...
		//first, find in currentProbability[i] which ones have 1.00 probability
		...
		while(currentColoring[i].size()<numFragment){
...
bool MultiCFL::ReconfigureProbability(double beta)  //Return true if there are no conflict (coloring found), false otherwise.
{
	...
	for(int i=0;i<num_nodes;i++){
		...
		NormalizeVector(prob, (1-beta)*((double)numFragment-N_NOCONF));

		for(int j=0; j<prob.size();j++){
			if(currentColoring[i].find(j)==currentColoring[i].end()) {
				prob[j] += beta * ((double)numFragment-N_NOCONF) / (numFragment*numColors-numFragment); //(M-N_NOCONF)/(NCOLOR_FRAC-M)
			}
			else {
				if(nconflict_set.find(j)!=nconflict_set.end()){
					prob[j] = 1.0;
...
double MultiCFL::NormalizeVector(vector<double> &obj, double sumTo)
{
	double sum = 0;
